...
- `[2026-06-16] CSE "blocked"/"no access": la clave AIza (Maps Platform key) tenía Restricción de API = solo "Places API". Hay que AÑADIR "Custom Search API" a esa clave (Cloud Console → Credenciales → la clave → Restricciones de API → marcar las dos) + tener la API habilitada. "Restricciones de aplicación" = Ninguno (la app corre en content://, sin referrer). El cambio de clave tarda unos minutos en propagar. El botón 🔍 Probar (v92) ahora compara WORKER vs CSE en el mismo bar.`
- `[2026-06-16] GRAVE para Tenerife/otras islas: la app nació SOLO para Gran Canaria y tenía "Gran Canaria" a fuego en TODAS las búsquedas → al meter bares de Tenerife: (a) las queries de CSE/Places buscaban "ciudad de Tenerife + Gran Canaria" (dos islas) → traía OTROS bares; (b) los prompts de Gemini decían "está en Gran Canaria, si es de otra isla devuelve vacío" → devolvía VACÍO en Tenerife (no sacaba emails); (c) el filtro esGC() de auto1/googleFill marcaba TODO bar no-GC como "otra zona - revisar" y no lo rellenaba. v93: usar zonaDe(l) (que ya deduce la isla por la ciudad) en mapsUrl/CSE/Places/prompts Gemini, y esGC SOLO si el lead es de GC. Regla: NADA de islas a fuego; usar zonaDe(l).`
