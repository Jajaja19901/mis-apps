...
function mat_capturarTodas(ahora) {
  ...
  for (let i = 0; i < tracks.length; i++) {
    ...
    // 🚗 Coche visto DE PERFIL (caja mucho más ancha que alta) → NO se ve la
    // matrícula, solo la rueda/lateral. Se descarta: la placa solo es legible de
    // frente o de atrás (el coche de delante mientras conduces), donde la caja es
    // casi cuadrada o más alta. Esto evita fotografiar la RUEDA del coche de al lado.
    if (t.clase !== 'motorcycle' && t.caja.al > 0 && (t.caja.an / t.caja.al) > 1.6) continue;
