...
function mat_fotografiar(veh, ahora) {
  ...
  m.cola.push({ cnv: cnv, ts: ahora, zona: 'vehiculo', fotoId: null, clase: (veh && veh.clase) || 'car' });
...
async function mat_procesarCola() {
  ...
  try {
    ...
    if (cand) {
      ...
      // Recién AHORA se guarda la foto (había matrícula de verdad): así la galería
      // solo tiene coches con placa leída, nunca ruedas ni laterales sin placa.
      const fotoId = foto.fotoId || mat_fotoGuardar(foto.cnv, foto.ts, { clase: foto.clase || 'car' });
