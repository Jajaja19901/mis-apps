...
function gesto_analizarPose(fuente, ts, tracks) {
  ...
  // frames. NADA más agresivo: seguir los brazos DE CERCA es lo que pilla la
  // mano al bolsillo (el límite por tiempo de antes se comía las metidas).
...
function gesto_evaluarOcultacion(trk, puntos, ts) {
  ...
  for (let k = 0; k < munecas.length; k++) {
    ...
    // «COGER» de verdad = brazo extendido A ALTURA DE ESTANTE (a la altura de
    // la cadera o por encima). Un brazo que pasa extendido POR DEBAJO de la
    // cadera (balanceo, meterse la mano al bolsillo desde abajo) NO es coger:
    // armaba ciclos falsos y las metidas de mano VACÍAS puntuaban como robo.
    if (dT > GESTO_EXT_ALCANCE * anchoHombros &&
        muneca.y <= caderasC.y + anchoHombros * 0.25) extendida = true;
    ...
    case 'ocultando':
      if (cerca) {
        ...
        if (dwell >= permanenciaMs && !m.completado) {
          ...
          if (manoOk) {
            m.completado = true;
            // 🎯 LA FIRMA DEL ROBO: coger → esconder CON UN OBJETO visto en esa
            // mano hace un momento → alerta DIRECTA (umbral de golpe). Meterse
            // la mano al bolsillo sin haber cogido nada suma lo normal — así el
            // aviso cae en la metida del objeto (la 3ª del vídeo del dueño), no
            // en las vacías de antes.
            const om = g.objetoEnMano && g.objetoEnMano[id];
            const conObjeto = !!(om && ts - om.ts <= GESTO_OBJETO_CADUCA_MS);
            let puntos_ciclo = GESTO_PTS_CICLO;
            if (conObjeto || estado.cfg.ocultacionUnGesto) {
              puntos_ciclo = Math.max(GESTO_PTS_CICLO * 2, estado.cfg.ocultacionUmbral || 60);
            }
