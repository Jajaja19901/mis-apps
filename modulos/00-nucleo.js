...
const CONFIG = {
  ...
  VERSION: '4.16',   // súbela con cada entrega: se ve en Ajustes → Sistema
};
...
const CFG_DEFECTOS = {
  ...
  soloRobos: false,         // 🎯 solo avisar de ROBOS (ocultación): silencia correr, agacharse, caídas, merodeo, aglomeración, colas… (deja robo + sabotaje + prueba)
  telegramToken: '',
  telegramChat: '',
...
/* En modos de COCHE (carretera/copiloto) se analiza a 704 px: un coche que
 * viene DE FRENTE a 30 m mide ~15-20 px a 512 (invisible para el modelo) y
...
function nuc_analisisMax() {
  // TODOS los modos de escena analizan a 704 px. Una persona LEJOS en la tienda
  // (la que roba al fondo), o un coche/cuchillo pequeño, mide ~20-30 px a 512 —
  // invisible para el modelo — y el doble a 704, donde SÍ se encuadra. La IA
  // corre en hilo aparte (⧉ Supercerebro/Potente), así que el coste extra apenas
  // toca la fluidez de la cámara. (El Básico/COCO usa su propia copia a 512.)
  return 704;
}
...
function nuc_frameAnalisis(fuente) {
  try {
    const lim = nuc_analisisMax();
    const w = estado.video.w || 0, h = estado.video.h || 0;
    if (!w || !h || w <= lim) return { fuente: fuente, escala: 1 };
    const factor = lim / w;
...
let nuc_srvEnVuelo = 0;          // cuántas fotos viajan al servidor ahora mismo
let nuc_srvUltimo = [];          // últimas cajas buenas que devolvió el servidor
let nuc_srvSeqEnv = 0;           // nº de la última foto ENVIADA
let nuc_srvSeqApl = 0;           // nº de la última respuesta ya dibujada
let nuc_srvUltimoEnvio = 0;      // cuándo se mandó la última (para no saturar)
let nuc_srvAviso = 0;            // para no llenar la consola de errores repetidos
const NUC_SRV_MAX_VUELO = 3;     // hasta 3 fotos a la vez → MUCHOS más análisis/segundo
const NUC_SRV_MIN_MS = 80;       // tope: no mandar más de ~12 fotos/segundo

/* Convierte lo que responde el servidor en cajas escaladas al vídeo. */
function nuc_srvEscalar(dets) {
  const w = estado.video.w || 640, h = estado.video.h || 480;
  const salida = [];
...
/* Manda UNA foto al servidor SIN esperar (dispara y sigue). Deja hasta
 * NUC_SRV_MAX_VUELO viajando a la vez → así salen muchos más análisis por
 * segundo (el móvil no se queda parado esperando la respuesta anterior). */
function nuc_srvLanzar() {
  const url = (estado.cfg.servidorUrl || '').trim();
  if (!url) return;
  if (nuc_srvEnVuelo >= NUC_SRV_MAX_VUELO) return;        // ya hay bastantes en el aire
  const ahora = Date.now();
  if (ahora - nuc_srvUltimoEnvio < NUC_SRV_MIN_MS) return; // no saturar la red/servidor
  const jpeg = (typeof vid_capturaJPEG === 'function') ? vid_capturaJPEG(640, 0.6) : null;
  if (!jpeg) return;
  nuc_srvUltimoEnvio = ahora;
  const seq = ++nuc_srvSeqEnv;
  nuc_srvEnVuelo++;
  ...
  fetch(url, {
    ...
    const salida = nuc_srvEscalar(data && (data.detecciones || data.detections));
    // Con varias fotos en vuelo pueden llegar desordenadas: dibujamos SOLO si
    // esta respuesta es más nueva que la última pintada (nunca pisar lo reciente).
    if (seq >= nuc_srvSeqApl) { nuc_srvSeqApl = seq; nuc_srvUltimo = salida; }
  }).catch(function (e) {
    const a = Date.now();
    if (a - nuc_srvAviso > 5000) { nuc_srvAviso = a; console.warn('[servidor] ' + (e && e.message)); }
  }).finally(function () {
    if (to) clearTimeout(to);
    nuc_srvEnVuelo--;
  });
...
/* 🖥️ MOTOR "SERVIDOR EN LA NUBE": el móvil NO piensa nada. En cada vuelta manda
 * una foto NUEVA al ordenador potente (si hay hueco) y DEVUELVE AL INSTANTE las
 * últimas cajas que ya llegaron — sin esperar. Así el vídeo va fluido y se
 * analiza muchas veces por segundo. El servidor responde cajas NORMALIZADAS
 * (0-1) que aquí se escalan al vídeo. NUNCA lanza. */
async function nuc_detectarServidor() {
  if (!(estado.cfg.servidorUrl || '').trim()) return [];
  nuc_srvLanzar();          // dispara una foto nueva si hay hueco (no espera)
  return nuc_srvUltimo;     // al instante: las últimas cajas buenas del servidor
}
...
function nuc_init() {
  ...
  // (Arranque en Básico por defecto: es RÁPIDO y detecta bien. El Supercerebro
  // en muchos móviles cae a modo lento y arrastraba la app; se elige a mano en
  // Ajustes → Detección → Motor quien quiera máxima precisión y tenga hardware.)

  // Migración única: DEJARLO RÁPIDO Y FUNCIONANDO SIN TOCAR NADA. El dueño no
  // debería pelear con los ajustes: si venía en un motor pesado (Supercerebro/
  // Potente) que ahoga su móvil, se pasa al Básico rápido; y la resolución de
