...
function trk_reiniciar() {
  ...
  for (let i = 0; i < previos.length; i++) {
    if (previos[i]) bus.emit('track:perdido', { track: previos[i] });
  }
  estado.trk.pistas = [];
  estado.tracks = [];
}
...
function trk_actualizar(detecciones, ts) {
  ...
  // --- estado.tracks = SOLO los visibles este frame ---
  const visibles = [];
  for (let i = 0; i < S.pistas.length; i++) {
    if (S.pistas[i].framesPerdidos === 0) visibles.push(S.pistas[i]);
  }
  estado.tracks = visibles;

  // --- Eventos (el bus envuelve cada oyente en try/catch) ---
  for (let i = 0; i < perdidos.length; i++) bus.emit('track:perdido', { track: perdidos[i] });
