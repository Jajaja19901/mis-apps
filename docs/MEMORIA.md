## 2026-08-23 — FIX crítico: las visuales no reaccionaban ("no hace nada")
- El cliente repetía "no hace nada": el MH y los presets no reaccionaban. Causa DOBLE: (1) el AudioContext arranca 'suspended' hasta un gesto y no se reanudaba al pulsar demo (launch early-return); (2) la fuente de demo no llegaba a destino → el grafo no se procesa → analizadores a cero. (3) BONUS de flujo: el build lee `tools/mh-visuals-template.html` y yo editaba la copia de scratchpad, así que los fixes no entraban al build.
- Solución: `resumeAudio()` en cada gesto (click/touch/botones); demo conectado a destino con gain 0.0008 (inaudible, mantiene el grafo vivo); en modo demo el bajo/beat se SINTETIZAN (kick limpio 126 BPM) para que el MH lata aunque no haya micro; meterAn smoothing 0.6. Validado headless con --autoplay-policy: bajo 0.12→0.92, 8 golpes/2.3s, beatHold 0.84 → REACCIONA ✅. Verificador APTO.
- IMPORTANTE para futuras sesiones: editar `tools/mh-visuals-template.html` (no la copia de scratchpad) y luego `node tools/build-mh-visuals.mjs`.
