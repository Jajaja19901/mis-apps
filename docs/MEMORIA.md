...
- Pendiente / siguiente paso: para que reaccione en directo, enrutar salida de mesa → entrada de línea/micro (o loopback tipo BlackHole/VB-Cable). Si dan el logo vectorial oficial, sustituir el emblema recreado en canvas por el real.
- Datos a confirmar: logo recreado a partir de la captura de Instagram (@mhcollective_); si tienen vectorial oficial, incrustarlo.
