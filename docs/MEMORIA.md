...
- Pendiente / siguiente paso: están en INGLÉS y asumen contexto SaaS (MRR, churn, freemium); si se usan con clientes locales españoles hay que traducir la salida y traducir métricas (ticket medio, clientes/mes). Valorar traducir/adaptar un subconjunto (estrategia + precios + ofertas) y engancharlo a `crear-app` y `captacion-leads`. Ojo: `content-strategy` del plugin solapa con la `content-strategy-sms` ya instalada.
