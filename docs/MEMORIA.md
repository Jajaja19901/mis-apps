...
- Qué se hizo: notebook Colab (GPU T4) que transcribe vídeo SIN audio con el repo mpc001/Visual_Speech_Recognition_for_Multiple_Languages. Inferencia only, MediaPipe (no dlib), Gradio share=True, descarga de pesos por idioma con gdown bajo demanda. Construido por fases con verificación. NO ejecutado en Colab desde aquí (sin GPU en el entorno): la verificación real la corre el usuario en Colab. Datos reales sacados del repo (configs, enlaces bit.ly→Drive, WER).
